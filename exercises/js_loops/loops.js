...
document.querySelector("#btn-count").onclick = (event) => {
  ...
  if (event.currentTarget.innerHTML.toLowerCase() == "start") {
    ...
    updateCount = setInterval(() => {
      count++;
      document.querySelector("#btn-count").innerHTML = count;
    }, 300);
  }
  //stop the counter
  else {
...
document.getElementById("show-fruit-btn").onclick = (event) => {
  event.currentTarget.disabled = true;


  //Stupid Dumb way
  const fruits = {"peach" : "tasty", "strawberry": "yum",
     "pinapple" : "strong", "Mango": "sweet"};
  const fruitBasket = document.getElementById("fruit-basket");
  //Based way
  fruits.forEach((toy, i)=>{
    const fruitLi = document.createElement("li")
    fruitLi.innerHTML = `${toy.key}, ${toy.value}`;
    fruitBasket.append(fruitLi);
  })
